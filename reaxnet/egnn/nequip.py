"""This file is a modified version of the original file from the jax-md library.
...
import reaxnet.egnn.nn_util as nn_util
from .e3nn_layer import FullyConnectedTensorProductE3nn, Linear
...
class NequIPEnergyModel(nn.Module):
  ...
  def __call__(self, vectors, species, senders, receivers):
    ...
    # node features
    #nodes = jax.nn.one_hot(species, self.n_elements)
    embedding_irreps = Irreps(f'{self.n_elements}x0e')
    node_attrs = IrrepsArray(embedding_irreps, species)
    ...
    #h_node = Linear(irreps_out=Irreps('32x0e'))(node_attrs)
    #h_node = Linear(irreps_out=Irreps(hidden_irreps.filter("0e").regroup()))(node_attrs)
